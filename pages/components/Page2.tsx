...
const Page2: React.FC<Page2Props> = ({ onBack, selectedDrink }) => {
  ...
  // Hàm render Robusta
  const renderRobustaOptions = (
    options: number[],
    robusta: number,
    setRobusta: (val: number) => void
  ) => (
            ...
            {[10, 15, 20, 25].map((val) => (
              <label key={val} style={{ marginRight: 16, fontWeight: 500 }}>
                <input
                  ...
                  name="condensedMilk"
                  value={val}
                  checked={condensedMilk === val}
                  onChange={() => setCondensedMilk(val)}
                />{" "}
                {val}g
              </label>
            ))}
            ...
            {[10, 15, 20, 25].map((val) => (
              <label key={val} style={{ marginRight: 16, fontWeight: 500 }}>
                <input
                  ...
                  name="freshMilk"
                  value={val}
                  checked={freshMilk === val}
                  onChange={() => setFreshMilk(val)}
                />{" "}
                {val}g
              </label>
            ))}
    ...
    <>
      ...
      <form
        ...
        style={{
          ...
          background: "#f7ede2",
          borderRadius: 20,
          boxShadow: "0 4px 24px rgba(182,137,76,0.10)",
          padding: 32,
          margin: "0 auto",
          position: "relative",
      ...
      >
        ...
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          <img
            src={DRINK_IMAGES[drink] || "/image/bacxiu.png"}
            alt={drink}
            style={{
              width: 100,
              height: 100,
              objectFit: "contain",
              borderRadius: 16,
              boxShadow: "0 2px 8px rgba(182,137,76,0.08)",
            }}
          />
        </div>
        <div
        ...
        >
          Giá:{" "}
          <span style={{ color: "#b6894c", fontWeight: 700 }}>
          ...
          <>
            <div style={{ marginBottom: 18 }}>
              ...
              {(() => {
                let options: number[] = [];
                if (drink === "Bạc Xỉu" || drink === "Nâu Đá") {
                  options = size === "M" ? [2, 5, 7, 10] : [3, 8, 11, 15];
                } else if (drink === "Espresso") {
                  options = size === "M" ? [6, 8, 10] : [10, 12, 14];
                }
        ...
        <button
          ...
          style={{
            ...
            padding: 14,
            borderRadius: 12,
            background: loading ? "#e0c9a5" : "#b6894c",
            color: "#fff",
            fontWeight: 700,
            fontSize: 20,
            border: "none",
          ...
          onMouseOver={(e) => {
            if (!loading) e.currentTarget.style.background = "#a06d2c";
          }}
          onMouseOut={(e) => {
            if (!loading) e.currentTarget.style.background = "#b6894c";
          }}
          disabled={loading}
        >
